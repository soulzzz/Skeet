...
public:
    static std::unordered_map<int, int> GetLocalBackpackItems(VMMDLL_SCATTER_HANDLE hScatter)
    {
        ...
        for (auto& Item : EqItemInfo)
        {
            ...
            else if (EntityInfo.DisplayName == "闪光弹" && Item.ItemCount >= GameData.Config.Item.FlashGrenade)
            {
                FilteredItems[Item.ItemID] = Item.ItemCount;
            }
            else if (EntityInfo.DisplayName == "烟雾弹" && Item.ItemCount >= GameData.Config.Item.SmokeGrenade)
            {
                FilteredItems[Item.ItemID] = Item.ItemCount;
            }
            else if (EntityInfo.DisplayName == "燃烧瓶" && Item.ItemCount >= GameData.Config.Item.MolotovGrenade)
            {
                FilteredItems[Item.ItemID] = Item.ItemCount;
            }
            else if (EntityInfo.DisplayName == "蓝圈手雷" && Item.ItemCount >= GameData.Config.Item.BluezoneGrenade)
            {
                FilteredItems[Item.ItemID] = Item.ItemCount;
            }
            else if (EntityInfo.DisplayName == "医疗箱" && Item.ItemCount >= GameData.Config.Item.Medicalkit)
            {
                FilteredItems[Item.ItemID] = Item.ItemCount;
            }
            else if (EntityInfo.DisplayName == "急救包" && Item.ItemCount >= GameData.Config.Item.FirstAidKit)
            {
                FilteredItems[Item.ItemID] = Item.ItemCount;
            }
            else if (EntityInfo.DisplayName == "绷带" && Item.ItemCount >= GameData.Config.Item.Bandage)
            {
                FilteredItems[Item.ItemID] = Item.ItemCount;
            }
            else if (EntityInfo.DisplayName == "肾上腺激素" && Item.ItemCount >= GameData.Config.Item.Epinephrine)
            {
                FilteredItems[Item.ItemID] = Item.ItemCount;
            }
            else if (EntityInfo.DisplayName == "能量饮料" && Item.ItemCount >= GameData.Config.Item.EnergyDrink)
            {
                FilteredItems[Item.ItemID] = Item.ItemCount;
            }
            else if (EntityInfo.DisplayName == "止疼药" && Item.ItemCount >= GameData.Config.Item.PainKiller)
            {
                FilteredItems[Item.ItemID] = Item.ItemCount;
            }
        }
