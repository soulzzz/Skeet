...
struct tab_state
{
    ImVec4 text, icon_color,bg,mark;
};
...
bool ImGui::Tabs(bool selected, const char* icon, const char* label, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    state.text = ImLerp(state.text, selected ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);
    state.bg = ImLerp(state.bg, selected ? c::accent : hovered ? ImColor(0, 0, 0, 0).Value : ImColor(0, 0, 0, 0).Value, g.IO.DeltaTime * 6.f);



    GetWindowDrawList()->AddText(font::icomoon_menu, 17, ImVec2(bb.Min.x + size_arg.x / 2 - icon_size.x / 2, bb.Min.y + 5), GetColorU32(state.text), icon);
    GetWindowDrawList()->AddText(font::calibri_bold, 12, ImVec2(bb.Min.x + size_arg.x / 2 - label_size.x / 2, bb.Min.y + size_arg.y - 15 - label_size.y), GetColorU32(state.text), label);
    GetWindowDrawList()->AddShadowRect(ImVec2(bb.Min.x, bb.Max.y - 2), bb.Max, GetColorU32(state.bg), 68, ImVec2(0, 0), NULL, 8);

    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y - 2), bb.Max, GetColorU32(state.bg), 8);


    return pressed;
}

bool ImGui::SubTabs(bool selected, const char* label, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.bg = ImLerp(it_anim->second.bg, selected ? c::accent : c::tabs::line, g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y-2), bb.Max, GetColorU32(it_anim->second.bg));
    GetWindowDrawList()->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(it_anim->second.bg,0.0), GetColorU32(it_anim->second.bg,0.0), GetColorU32(it_anim->second.bg,0.2), GetColorU32(it_anim->second.bg,0.2));

    PushFont(font::calibri_regular);
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderText(bb.Min + (size - label_size) * 0.5f, label);
    PopStyleColor();
    PopFont();
...
bool ImGui::icon_box(ImFont* font, const char* icon, ImVec2 size, ImU32 color_bg, ImU32 color_icon, ImU32 color_border)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    ImGui::RenderTextColor(font, rect.Min, rect.Max, GetColorU32(color_icon), icon, ImVec2(0.5, 0.5));
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, GetColorU32(it_anim->second.background), 0.f);
...
{
    std::string label_key = "picker", label_bind = label_key + label;
    ImGui::SetCursorPos(GetCursorPos() - ImVec2(40, 0));
    ImGui::ColorEdit4(label_bind.c_str(), color, flags | ImGuiColorEditFlags_NoLabel);
...
bool ImGui::SliderScalarF(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 3), frame_bb.Max - ImVec2(0, 3), GetColorU32(c::slider::background), c::slider::rounding);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 3), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x, grab_bb.Max.y-2), GetColorU32(c::accent),  c::slider::rounding);
        GetWindowDrawList()->AddShadowRect(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x - 5, grab_bb.Min.y - 4), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 15, grab_bb.Max.y + 2), GetColorU32(c::shadow), 10, ImVec2(0, 0));
        GetWindowDrawList()->AddRectFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x - 5, grab_bb.Min.y - 4), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 15, grab_bb.Max.y + 2), GetColorU32(c::accent), c::slider::rounding);
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect clickable(rect.Min + ImVec2(width - 32, 0), rect.Max-ImVec2(0,1));
    ...
    window->DrawList->AddShadowRect(clickable.Min, clickable.Max, GetColorU32(c::shadow,0.65f), 10,ImVec2(0,0));

    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, GetColorU32(c::checkbox::background), c::picker::rounding);

    window->DrawList->AddRectFilled(clickable.Min+ImVec2(4,4), clickable.Max-ImVec2(4,4), GetColorU32(col_rgb), c::picker::rounding);

    RenderColorRectWithAlphaCheckerboard(window->DrawList, clickable.Min + ImVec2(4, 4), clickable.Max - ImVec2(4, 4), GetColorU32(col_rgb), 10, ImVec2(0.f, 0.f), c::picker::rounding);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.mark = ImLerp(it_anim->second.mark, selected ? c::accent : ImColor(0, 0, 0, 0).Value, g.IO.DeltaTime * 6.f);

    it_anim->second.move = ImLerp(it_anim->second.move, selected ? 20.f : hovered ? 15.f : 0.f, g.IO.DeltaTime * 8.f);
    ...
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(text_min+ImVec2(it_anim->second.move,-3), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
    PopStyleColor();
