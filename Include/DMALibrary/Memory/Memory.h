...
public:
	...
	*/
	 ...
	 */
	void AddScatterReadRequest1(VMMDLL_SCATTER_HANDLE handle, uint64_t address, void* buffer, size_t size);
	...
	void AddScatterRead(VMMDLL_SCATTER_HANDLE handle, uint64_t address, T* buffer) {
		AddScatterReadRequest1(handle, address, static_cast<void*>(buffer), sizeof(T));
	}
	template <typename T>
