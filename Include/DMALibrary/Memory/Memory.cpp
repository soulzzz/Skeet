...
bool Memory::Init(std::string process_name, bool memMap, bool debug)
{
	if (!DMA_INITIALIZED)
	{
		LOG("inizializing...\n");
		//-norefresh
		LPCSTR args[] = { (LPCSTR)"", (LPCSTR)"-device", (LPCSTR)"fpga://algo=0", (LPCSTR)"-norefresh", (LPCSTR)"", (LPCSTR)"", (LPCSTR)"" };
		DWORD argc = 4;
		if (debug)
		{
			args[argc++] = (LPCSTR)"-v";
...
DWORD Memory::GetTslGamePID()
{
	...
	for (auto pid : pid_list)
	{
		...
		if (UWorld > 0)
		{
			...
			GameData.GameBase = this->current_process.base_address;
			GameData.PID = this->current_process.PID;
			//GameData.PhysxInstancePtr = this->Read<uint64_t>(this->current_process.base_address + GameData.Offset["PhysxSDK"]);
			PROCESS_INITIALIZED = true;
...
void Memory::AddScatterReadRequest1(VMMDLL_SCATTER_HANDLE handle, uint64_t address, void* buffer, size_t size)
{
